...
function generateOverviewTable(sovereignEntity: SovereignEntity) {
  ...
  overviewTable += FormatUtils.formatRow([
    ...
    FormatUtils.formatNumberWithUnit(
      sovereignEntity.resolvedSize,
      sovereignEntity.plugin.configManager.geographyAreaUnit
    ),
  ]);
...
function generateTerritoryTable(sovereignEntity: SovereignEntity) {
  if (sovereignEntity.configuration.geography.territories.length === 1) {
    Logger.warn(sovereignEntity, "No territories defined.");
    return "";
  }
  ...
  for (const territory of sovereignEntity.configuration.geography.territories) {
    ...
    const territoryCount = sovereignEntity.resolvedSize / averageSize;
    const averagePopulation = sovereignEntity.population / territoryCount;
    territoryTable += FormatUtils.formatRow([
      territory.type,
      territoryCount,
      FormatUtils.formatNumberWithUnit(averageSize, sovereignEntity.plugin.configManager.geographyAreaUnit),
      averagePopulation,
    ]);
  }
